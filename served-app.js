...
/**
 * Draw a simple simulated heatmap into #google-heatmap when Google Maps is unavailable.
 */
function initHeatmapFallback() {
    try {
        const container = document.getElementById('google-heatmap');
        if (!container) return;
        container.innerHTML = '';
        ...
        function drawHeat() {
            ...
            const blobs = [
                { x: canvas.width * 0.35, y: canvas.height * 0.4, r: 120, a: 0.6 },
                { x: canvas.width * 0.6, y: canvas.height * 0.5, r: 90, a: 0.5 },
                { x: canvas.width * 0.5, y: canvas.height * 0.25, r: 70, a: 0.4 }
            ];

            blobs.forEach((b, i) => {
                const grd = ctx.createRadialGradient(b.x, b.y, 0, b.x, b.y, b.r);
                grd.addColorStop(0, `rgba(255,0,0,${b.a})`);
                grd.addColorStop(0.5, `rgba(255,165,0,${b.a * 0.6})`);
        ...
        const anim = setInterval(() => {
            ...
            canvas.width = containerRect.width || canvas.width;
            canvas.height = containerRect.height || canvas.height;
            // jitter blobs
            drawHeat();
        }, 2500);

        // store for potential cleanup
        container.dataset.heatmapAnim = String(true);
        console.log('[SmartVenue] Fallback heatmap initialized');
    } catch (err) {
        reportErrorToAnalytics(err, 'init_heatmap_fallback');
    }
}

/**
 * Initialize heatmap container: use Google Maps heatmap when available, otherwise fallback.
 */
function initHeatmapContainer() {
    try {
        ...
        if (typeof google !== 'undefined' && google.maps && google.maps.visualization) {
            // create map inside heatmap container
            const mapOptions = {
            ...
            const heatmap = new google.maps.visualization.HeatmapLayer({
                data: [new google.maps.LatLng(40.7505, -73.9934)],
                map: map,
                radius: 40,
                opacity: 0.6
            });
            ...
            window.venueMapHeat = map;
            console.log('[SmartVenue] Google heatmap initialized in container');
            // try to fetch ML updates
            updateHeatmapFromML();
        } else {
            initHeatmapFallback();
        }
    } catch (error) {
        reportErrorToAnalytics(error, 'init_heatmap_container');
    }
}
...
function initStadiumMap() {
    try {
        ...
        let mapContainer = document.getElementById('google-heatmap');
        if (!mapContainer) {
            mapContainer = document.createElement('div');
            mapContainer.id = 'google-heatmap';
...
function initGoogleMaps() {
    try {
        // If Google Maps JS is loaded asynchronously it may not be available
        // at DOMContentLoaded. Wait for the `google.maps` object and
        // the visualization library before initializing the stadium map.
        if (typeof google === 'undefined' || !google.maps || !google.maps.visualization) {
            console.log('[SmartVenue] Waiting for Google Maps API to load...');
            ...
            // Safety timeout: try for up to 10 seconds then stop waiting
...
function initLazyLoadAssets() {
    ...
    const lazyMapContainer = document.getElementById('google-heatmap');
    const mainMapContainer = document.getElementById('google-map');
    ...
    if (lazyMapContainer) observer.observe(lazyMapContainer);
    if (mainMapContainer) observer.observe(mainMapContainer);
